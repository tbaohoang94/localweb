...
const CustomActivitiesView = dynamic(
  () => import("@/components/dashboard/views/custom-activities-view"),
  { ssr: false }
);
const LogsView = dynamic(
  () => import("@/components/dashboard/views/logs-view"),
  { ssr: false }
);
...
export default function DashboardPage() {
  ...
  const triggerSync = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Sync fehlgeschlagen:", body.error ?? res.status);
      }
    } catch (err) {
      console.error("Sync-Request fehlgeschlagen:", err);
    }
    // Webhook antwortet sofort (onReceived), Sync laeuft im Hintergrund ~2-3 Min
  ...
  const filterConfig: Record<ViewId, { showRep: boolean; repOptions: string[] }> = {
    ...
    "custom-activities": { showRep: true, repOptions: repOptions.all },
    logs: { showRep: false, repOptions: repOptions.all },
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="animate-fade-in" key={view}>
            ...
            {view === "custom-activities" && <CustomActivitiesView filters={filters} />}

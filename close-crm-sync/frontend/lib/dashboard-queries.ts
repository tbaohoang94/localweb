...
import type {
  ...
  CloserProvisionSummary, SyncLogEntry, CloserPipelineRow,
  CustomActivityRow, CustomActivityKPIs,
...
export async function fetchCloserKPIs(
  ...
  // SG-Leads pro Closer: welche Leads hatten ein SG stattgefunden?
  async function buildSgLeadMap(): Promise<Map<string, Set<string>>> {
    const map = new Map<string, Set<string>>();
    if (!sgTypeId || closerIds.length === 0) return map;
    let q = supabase
      .from("custom_activities")
    ...
    for (const row of data ?? []) {
      ...
      map.get(row.user_id)!.add(row.lead_id);
    }
    return map;
  }

  const [egActivityMap, sgActivityMap, egNoShowMap, sgNoShowMap, sgLeadMap] = await Promise.all([
...
export async function fetchRevenueByMonth(
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const label = d.toLocaleString("de-DE", { month: "short", timeZone: "Europe/Berlin" });
...
export async function fetchWeeklyHistory(
  ...
  // count-basierte Aggregation statt alle Rows laden (calls kann 50k+ in 6 Wochen haben)
  const [callsCount, meetingsCount] = await Promise.all([
    supabase.from("calls").select("close_created_at").gte("close_created_at", since).order("close_created_at").limit(10000),
    supabase.from("meetings").select("close_created_at").gte("close_created_at", since).order("close_created_at").limit(10000),
  ]);
  const callsRes = callsCount;
  const meetingsRes = meetingsCount;
...
export async function fetchColdcallerCalls(
  ...
  if (callIds.length > 0) {
    const { data: transcripts } = await supabase
      .from("transcripts")
      .select("source_id, google_drive_url")
...
export async function fetchSettingActivities(
  ...
  const { data: typeRows } = await supabase
    .from("custom_activity_types")
    .select("id")
    .eq("close_type_id", CLOSE_ACTIVITY_TYPES.ERSTGESPRAECH)
    .limit(1);

  const egTypeId = typeRows?.[0]?.id;
  if (!egTypeId) return [];

  let query = supabase
    .from("custom_activities")
    .select("id, lead_id, user_id, custom_fields, close_created_at, leads(lead_name, close_lead_id)")
    .eq("type_id", egTypeId)
    .is("deleted_at", null)
    .gte("close_created_at", filters.from)
    .lte("close_created_at", filters.to + "T23:59:59")
...
/* ─── CLOSER PROVISION SUMMARY (alle Closer nebeneinander) ─── */

export async function fetchCloserProvisionSummary(
  supabase: SupabaseClient,
  filters: DashboardFilters
): Promise<CloserProvisionSummary[]> {
  ...
  let wonQuery = supabase
    .from("opportunities")
    .select("value, monthly_value, setup_fee, contract_duration, custom_fields, closed_at, user_id")
    .ilike("status", "%Won%")
    .gte("closed_at", filters.from)
    .lte("closed_at", filters.to + "T23:59:59");

  const { data: wonDeals, error } = await wonQuery;
  if (error) throw new Error(error.message);
  ...
  return closers.map((closer) => {
    const myWon = (wonDeals ?? []).filter((o) => o.user_id === closer.id);
    let setupFeeTotal = 0;
    let yearlyRevenue = 0;
    let empfehlungTotal = 0;
    let provisionTotal = 0;

    for (const deal of myWon) {
      const fin = oppFinancials(deal);
      setupFeeTotal += fin.setupFee;
      yearlyRevenue += fin.monthlyValue * 12;
      const src = ((deal.custom_fields as Record<string, unknown> | null)?.[srcKey] as string) ?? "";
      const empfBonus = src.toLowerCase().includes("empfehlung") ? 200 : 0;
      empfehlungTotal += empfBonus;
      provisionTotal += closerRule
        ? Math.round(0.5 * fin.setupFee + (fin.contractDuration / 6) * fin.monthlyValue + empfBonus)
        : (deal.value ?? 0);
    }
  ...
  }).sort((a, b) => b.provisionTotal - a.provisionTotal);
}
...
/* ─── CLOSER PIPELINE (Opportunities pro Closer) ─── */

export async function fetchCloserPipeline(
  supabase: SupabaseClient,
  filters: DashboardFilters
): Promise<CloserPipelineRow[]> {
  ...
  const result: CloserPipelineRow[] = [];

  for (const closer of closers) {
    const myOpps = opps.filter((o) => o.user_id === closer.id);
    let egStattgefunden = 0;
    let sgStattgefunden = 0;
    let angebotVerschickt = 0;
    let won = 0;
    let lost = 0;
    let pipelineValue = 0;
    ...
    for (const opp of myOpps) {
      const stage = mapStatus(opp.status);
      switch (stage) {
        case "EG stattgefunden": egStattgefunden++; break;
        case "SG stattgefunden": sgStattgefunden++; break;
        case "Angebot verschickt": angebotVerschickt++; break;
        case "Won": won++; break;
        case "Lost": lost++; break;
      }
      if (isActiveStage(stage)) pipelineValue += opp.value ?? 0;
    }
...
/** Alle Activity Types laden */
export async function fetchActivityTypes(
  supabase: SupabaseClient,
): Promise<{ id: string; name: string; closeTypeId: string }[]> {
  const { data, error } = await supabase
    .from("custom_activity_types")
    .select("id, name, close_type_id")
    .order("name");
  if (error) throw new Error(error.message);
  return (data ?? []).map((t) => ({ id: t.id, name: t.name, closeTypeId: t.close_type_id }));
}

/** Custom Activities mit Joins laden */
export async function fetchCustomActivities(
  supabase: SupabaseClient,
  filters: { from: string; to: string; rep: string },
): Promise<CustomActivityRow[]> {
  const toDate = `${filters.to}T23:59:59`;
  let q = supabase
    .from("custom_activities")
    .select("id, type_id, user_id, close_created_at, custom_activity_types(name), users(first_name, last_name), leads(lead_name, close_lead_id)")
    .is("deleted_at", null)
    .gte("close_created_at", filters.from)
    .lte("close_created_at", toDate)
    .order("close_created_at", { ascending: false })
  ...
  return (data ?? []).map((a: any) => ({
    id: a.id,
    closeLeadId: a.leads?.close_lead_id ?? "",
    leadName: a.leads?.lead_name ?? "–",
    activityType: a.custom_activity_types?.name ?? "–",
    updatedAt: a.close_created_at ?? "",
    createdBy: userName(a.users?.first_name ?? "", a.users?.last_name ?? ""),
    userId: a.user_id,
    typeId: a.type_id,
  }));
}

/** KPI-Zaehler fuer die wichtigsten Activity-Typen */
export async function fetchActivityKPIs(
  supabase: SupabaseClient,
  filters: { from: string; to: string; rep: string },
): Promise<CustomActivityKPIs> {
  const toDate = `${filters.to}T23:59:59`;

  // Alle relevanten close_type_ids fuer Lookup
  const kpiTypeIds = [
    CLOSE_ACTIVITY_TYPES.EG_VEREINBART_ET,
    CLOSE_ACTIVITY_TYPES.EG_VEREINBART_FT,
    CLOSE_ACTIVITY_TYPES.EG_NOSHOW,
    ...
    CLOSE_ACTIVITY_TYPES.STRATEGIEGESPRAECH,
    CLOSE_ACTIVITY_TYPES.SG_NOSHOW,
    CLOSE_ACTIVITY_TYPES.KUNDE_GEWONNEN,
  ];

  // close_type_id → UUID Map
  const { data: types, error: typesErr } = await supabase
    .from("custom_activity_types")
    .select("id, close_type_id")
    .in("close_type_id", kpiTypeIds);
  if (typesErr) throw new Error(typesErr.message);

  const typeMap = new Map<string, string>();
  for (const t of types ?? []) typeMap.set(t.close_type_id, t.id);

  const uuidFor = (closeId: string) => typeMap.get(closeId);

  // EG vereinbart = ET + FT
  const egVereinbartEtId = uuidFor(CLOSE_ACTIVITY_TYPES.EG_VEREINBART_ET);
  const egVereinbartFtId = uuidFor(CLOSE_ACTIVITY_TYPES.EG_VEREINBART_FT);
  const egNoShowId = uuidFor(CLOSE_ACTIVITY_TYPES.EG_NOSHOW);
  const egStattId = uuidFor(CLOSE_ACTIVITY_TYPES.ERSTGESPRAECH);
  const sgStattId = uuidFor(CLOSE_ACTIVITY_TYPES.STRATEGIEGESPRAECH);
  const sgNoShowId = uuidFor(CLOSE_ACTIVITY_TYPES.SG_NOSHOW);
  const kundeGewonnenId = uuidFor(CLOSE_ACTIVITY_TYPES.KUNDE_GEWONNEN);

  const allIds = [egVereinbartEtId, egVereinbartFtId, egNoShowId, egStattId, sgStattId, sgNoShowId, kundeGewonnenId].filter(Boolean) as string[];

  // Eine Query: alle relevanten Activities holen, dann client-side zaehlen
  let q = supabase
    .from("custom_activities")
    .select("type_id")
    .is("deleted_at", null)
  ...
  for (const a of activities ?? []) {
    counts.set(a.type_id, (counts.get(a.type_id) ?? 0) + 1);
  }

  const c = (id: string | undefined) => (id ? counts.get(id) ?? 0 : 0);

  return {
    egVereinbart: c(egVereinbartEtId) + c(egVereinbartFtId),
    egNoShow: c(egNoShowId),
    egStattgefunden: c(egStattId),
    sgStattgefunden: c(sgStattId),
    sgNoShow: c(sgNoShowId),
    kundeGewonnen: c(kundeGewonnenId),
  };
}

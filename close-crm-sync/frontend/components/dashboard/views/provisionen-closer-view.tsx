...
import { fetchCloserProvisionTransactions, fetchMonthlyProvision, fetchCloserProvisionSummary } from "@/lib/dashboard-queries";
import { useDashboardQuery } from "@/hooks/use-dashboard-data";
import DataTable from "@/components/dashboard/data-table";
import type { Filters } from "@/lib/constants";
import type { ProvisionTransaction, CloserProvisionSummary } from "@/lib/types/dashboard";
...
export default function ProvisionenCloserView({ filters }: { filters: Filters }) {
  ...
  const { data: summary, loading: l3, error: e3 } = useDashboardQuery(fetchCloserProvisionSummary, filters);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      {/* Uebersicht alle Closer */}
      {(summary ?? []).length > 0 && (
        <Card className="p-5">
          <p className="text-sm font-semibold mb-4">Provision pro Closer</p>
          <DataTable
            columns={[
              { key: "name", label: "Closer", bold: true },
              { key: "wonDeals", label: "Won Deals", right: true, mono: true },
              { key: "setupFeeTotal", label: "Einrichtung", right: true, mono: true, render: (v: number) => euro(v) },
              { key: "yearlyRevenue", label: "Lfd. Jahreseinnahmen", right: true, mono: true, render: (v: number) => euro(v) },
              { key: "empfehlungTotal", label: "Empfehlung", right: true, mono: true, render: (v: number) => v > 0 ? <span className="text-success font-semibold">{euro(v)}</span> : "–" },
              { key: "provisionTotal", label: "Provision", right: true, mono: true, render: (v: number) => <span className="text-success font-semibold">{euro(v)}</span> },
            ]}
            rows={[
              ...(summary ?? []),
              {
                userId: "_total",
                name: "Gesamt",
                wonDeals: (summary ?? []).reduce((s, r) => s + r.wonDeals, 0),
                setupFeeTotal: (summary ?? []).reduce((s, r) => s + r.setupFeeTotal, 0),
                yearlyRevenue: (summary ?? []).reduce((s, r) => s + r.yearlyRevenue, 0),
                empfehlungTotal: (summary ?? []).reduce((s, r) => s + r.empfehlungTotal, 0),
                provisionTotal: (summary ?? []).reduce((s, r) => s + r.provisionTotal, 0),
              },
            ]}
          />
        </Card>
